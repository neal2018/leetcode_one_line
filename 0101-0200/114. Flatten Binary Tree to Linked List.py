class Solution:
    def flatten(self, root: TreeNode) -> None:
        ...
        """
        (lambda node: (setattr(self, "next", None), (lambda f, node: f(f, node))(lambda f, node: node and (f(f, node.right), f(f, node.left), setattr(node, "left", None), setattr(node, "right", self.next), setattr(self, "next", node)), node)))(root)
